...
const DUMMY_TRENDING_HASHTAGS: HashtagData[] = [
    { uuid: "1", canonical_form: "productdesigner", display_form: "productdesigner", usage_count: "72000" },
    { uuid: "2", canonical_form: "techindustry", display_form: "techindustry", usage_count: "6500000" },
    { uuid: "3", canonical_form: "artificialintelligence", display_form: "artificialintelligence", usage_count: "3200000" },
    { uuid: "4", canonical_form: "love", display_form: "love", usage_count: "10000" },
    { uuid: "5", canonical_form: "zaroncosmetics", display_form: "zaroncosmetics", usage_count: "5000000" },
    { uuid: "6", canonical_form: "piggyvestapp", display_form: "piggyvestapp", usage_count: "2700" },
    { uuid: "7", canonical_form: "apple", display_form: "apple", usage_count: "18000000" },
];
...
const VFlixUploadModal = ({ user }: { user: any }) => {
    ...
    const [activeMentionSearch, setActiveMentionSearch] = useState<string | null>(null);
    ...
    useEffect(() => {
        ...
        const fetchMentions = async () => {
            ...
            try {
                const res = await fetch(`/api/mentions?q=${encodeURIComponent(searchTerm)}`);
                const data = await res.json();
                if (!isCurrent) return;
                if (data?.status && data?.data) {
    ...
    const insertTextToCaption = (textToInsert: string, type: "mention" | "hashtag") => {
        ...
        if (match) {
            ...
            const newCaption = caption.substring(0, index) + (type === "mention" ? `@${textToInsert} ` : `#${textToInsert} `) + textAfterCursor;
    ...
    return (
        <Dialog open={isVFlixUploadOpen} onOpenChange={(open) => !open && handleClose()}>
            <DialogContent className={`flex flex-col p-0 rounded-2xl overflow-hidden border border-white/5 bg-[#1C1C1E] shadow-2xl [&>button:last-child]:hidden transition-all duration-300 ${step === 1 ? "max-w-[450px]" : "max-w-[700px]"} w-full`}>

                {step === 1 && (
                    <div
                        className="flex flex-col items-center justify-center p-12 min-h-[350px] bg-[#161618]"
                    ...
                    >
                        ...
                        <h2 className="text-white text-lg font-semibold tracking-wide mb-2">Select Media to Upload</h2>
                        {/* <p className="text-gray-400 text-sm mb-8">Or drag photos and video here</p> */}

                        <Button
                            className="bg-[#0D52D2] hover:bg-[#0D52D2]/90 text-white px-10 rounded-full font-medium"
                ...
                {step === 2 && (
                    <div className="flex flex-col bg-[#161618]">
                        ...
                        <div className="flex flex-row p-6 gap-x-6 min-h-[350px]">
                            {/* Left Side: Video Preview */}
                            <div className="w-[45%] rounded-xl overflow-hidden bg-black flex shrink-0">
                                {videoPreviewUrl && (
                                    <video
                                        src={videoPreviewUrl}
                            ...
                            <div className="flex flex-col flex-1">
                                <div className="relative w-full min-h-[120px] mb-4">
                                    ...
                                    >
                                        {!caption ? (
                                            <span className="text-gray-400">Say something about this post...</span>
                                        ) : (
                                            caption.split(/(\s+)/).map((part, i) => {
                                                if (/^@[\w\d._]+/.test(part) || /^#[\w\d_]+/.test(part)) {
                                ...
                                <div className="flex flex-row gap-x-3 mb-6 relative">
                                    <button onClick={handleTagPeopleClick} className="flex items-center gap-x-2 bg-white/5 hover:bg-white/10 text-white text-xs font-medium px-4 py-2 rounded-full transition-colors">
                                        <span className="text-gray-400">@</span> Tag people
                                    </button>
                                    <button onClick={handleHashtagsClick} className="flex items-center gap-x-2 bg-white/5 hover:bg-white/10 text-white text-xs font-medium px-4 py-2 rounded-full transition-colors">
                                    ...
                                    {activeHashtagSearch !== null && (
                                        <div className="absolute top-full mt-2 left-0 w-[240px] z-30 bg-[#1A1A1C] border border-white/5 shadow-2xl rounded-xl max-h-[280px] overflow-y-auto no-scrollbar py-2">
                                            {loadingHashtags && hashtagResults.length === 0 && (
                                                <div className="py-4 text-center text-[13px] text-white/50">Searching...</div>
                                            )}
                                            {!loadingHashtags && hashtagResults.length === 0 && (
                                                <div className="py-4 text-center text-[13px] text-white/50">No hashtags found.</div>
                                            )}
                                            {hashtagResults.map((tag) => (
                                                <button
                                                    key={tag.uuid}
                                                    onClick={() => insertTextToCaption(tag.display_form, "hashtag")}
                                                    className="w-full flex flex-col px-4 py-2.5 hover:bg-white/5 transition-colors text-left"
                                                >
                                                    <span className="text-[14px] font-bold text-white line-clamp-1 mb-0.5">#{tag.display_form}</span>
                                                    <span className="text-[12px] text-white/50">{formatUsageCount(tag.usage_count)}</span>
                                                </button>
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    {activeMentionSearch !== null && (
                                        <div className="absolute top-full mt-2 left-0 w-[240px] z-30 bg-[#1A1A1C] border border-white/5 shadow-2xl rounded-xl max-h-[280px] overflow-y-auto no-scrollbar py-2">
                                            ...
                                            {!loadingMentions && mentionResults.length === 0 && activeMentionSearch.trim() && (
                                                <div className="py-4 text-center text-[13px] text-white/50">No users found.</div>
                                            )}
                                            {!loadingMentions && !activeMentionSearch.trim() && (
                                                <div className="py-4 text-center text-[12px] text-white/40">Type a username to search...</div>
                                            )}
                                            {mentionResults.map((user) => (
                                                <button
                                                    key={user.uuid}
                                                    onClick={() => insertTextToCaption(user.username, "mention")}
                                                    className="w-full flex items-center gap-3 px-3 py-2 hover:bg-white/5 transition-colors text-left"
                                                >
                                                    <CustomAvatar src={user.profile_picture} name={user.name} className="size-9 flex-shrink-0" />
                                                    <div className="flex flex-col">
                                                        <span className="text-[13px] font-bold text-white line-clamp-1 leading-snug">{user.name}</span>
                                                        <span className="text-[12px] text-white/50 line-clamp-1">@{user.username}</span>
                                                    </div>
                                                </button>
                                            ))}
                                        </div>
                                ...
                                <div className="flex flex-col gap-y-1 mb-auto relative">
                                    <button
                                        onClick={() => setIsPrivacyOpen(!isPrivacyOpen)}
                                        className={`flex items-center justify-between text-white hover:bg-white/5 p-3 transition-all ${isPrivacyOpen ? 'rounded-t-xl rounded-b-none bg-white/5' : 'rounded-xl'}`}>
                                        <div className="flex items-center gap-x-3">
                                            ...
                                            <span className="text-[14px] font-medium">Choose who can view</span>
                                        </div>
                                        <FaAngleRight className={`text-gray-500 transition-transform ${isPrivacyOpen ? 'rotate-90' : ''}`} />
                                    </button>
                                    ...
                                    {isPrivacyOpen && (
                                        <div className="absolute top-[48px] left-0 w-full z-20 bg-[#1A1A1C] border border-white/5 shadow-2xl rounded-b-xl flex flex-col pt-1 pb-2">
                                            <button
                                                onClick={() => { setPrivacySetting("Everyone"); setIsPrivacyOpen(false); }}
                                            ...
                                            >
                                                <div className="flex items-center gap-x-3">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <span className="text-white text-[13px] font-medium">Everyone</span>
                                                        <span className="text-gray-500 text-[11px]">Everyone can see your posts.</span>
                                                    </div>
                                                </div>
                                                <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${privacySetting === "Everyone" ? 'border-none bg-[#0D52D2]' : 'border-gray-500'}`}>
                                                    {privacySetting === "Everyone" && <div className="w-1.5 h-1.5 rounded-full bg-white"></div>}
                                                </div>
                                            </button>

                                            <button
                                                onClick={() => { setPrivacySetting("Friends / Followers"); setIsPrivacyOpen(false); }}
                                                className="flex items-center justify-between p-3 hover:bg-white/5 text-left transition-colors"
                                            >
                                                <div className="flex items-center gap-x-3">
                                                    <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <span className="text-white text-[13px] font-medium">Friends / Followers</span>
                                                        <span className="text-gray-500 text-[11px]">Limited to your friends or followers.</span>
                                                    </div>
                                                </div>
                                                <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${privacySetting === "Friends / Followers" ? 'border-none bg-[#0D52D2]' : 'border-gray-500'}`}>
                                                    {privacySetting === "Friends / Followers" && <div className="w-1.5 h-1.5 rounded-full bg-white"></div>}
                                                </div>
                                            </button>

                                            <button
                                                onClick={() => { setPrivacySetting("Only me"); setIsPrivacyOpen(false); }}
                                                className="flex items-center justify-between p-3 hover:bg-white/5 text-left transition-colors"
                                            >
                                                <div className="flex items-center gap-x-3">
                                                    <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <span className="text-white text-[13px] font-medium">Only me</span>
                                                        <span className="text-gray-500 text-[11px]">Only you can see this post</span>
                                                    </div>
                                                </div>
                                                <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${privacySetting === "Only me" ? 'border-none bg-[#0D52D2]' : 'border-gray-500'}`}>
                                                    {privacySetting === "Only me" && <div className="w-1.5 h-1.5 rounded-full bg-white"></div>}
                                                </div>
                                            </button>
                                        </div>
                                    )}

                                    <button className="flex items-center justify-between text-white hover:bg-white/5 p-3 rounded-xl transition-all">
                                        <div className="flex items-center gap-x-3">
                                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-gray-400">
                                                <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path>
                                                <circle cx="12" cy="10" r="3"></circle>
                                            </svg>
                                            <span className="text-[14px] font-medium">Add location</span>
                                        </div>
                                    ...
                                    <div className="flex items-center justify-between text-white p-3 rounded-xl">
                                        <div className="flex items-center gap-x-3">
                                            ...
                                            <span className="text-[14px] font-medium">Allow comments</span>
                                        </div>
