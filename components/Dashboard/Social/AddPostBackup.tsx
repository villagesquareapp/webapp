...
const AddPost = ({
        ...
        posts: [
          {
            ...
            latitude: newPost.latitude ? Number(newPost.latitude) : null,
            longitude: newPost.longitude ? Number(newPost.longitude) : null,
            privacy: newPost.privacy,
          },
        ],
      };
  ...
  const handleLargeFileUpload = async (file: File, fileId: string) => {
    try {
      ...
      console.log(
        `File details: Size=${(file.size / (1024 * 1024)).toFixed(2)}MB, Type=${
          file.type
        }`
      );
      ...
      for (let chunkIndex = 0; chunkIndex < totalChunks; chunkIndex++) {
        ...
        console.log(
          `Uploading chunk with offset: ${start}, checksum: ${checksum}`
        );
        ...
        const uploadPromise = uploadPostMediaGreaterThan6MB(formData).then(
          (response) => {
            ...
            if (response?.status && response?.data) {
              ...
              if (!partNumber || !eTag) {
                console.error(
                  "Invalid chunk upload response format:",
                  response.data
                );
        ...
        if (
          ...
          console.log(
            `Waiting for ${uploadPromises.length} chunks to complete...`
          );
      ...
      const sortedParts = uploadedParts
        .sort((a, b) => parseInt(a.partNumber) - parseInt(b.partNumber))
        .map((part) => ({
      ...
      console.log(
        "Completing upload with payload:",
        JSON.stringify(
          {
            filename: file.name,
            upload_id: uploadId,
  ...
  const handleChangePostImage = async (
  ...
  const calculateChecksum = async (chunk: Blob): Promise<string> => {
    ...
    try {
      ...
      const hashHex = SparkMD5.ArrayBuffer.hash(arrayBuffer);

      return hashHex;
    } catch (error) {
      console.error("Error calculating checksum:", error);
      // Fallback (won't match backend, but prevents crash)
      return `${chunk.size}-${Date.now()}`;
    }
  };
                ...
                <div className="h-full w-full bg-accent relative rounded-lg ">
                  ...
                  {previewImageForPost.file?.type.includes("video") ? (
                    ...
                    <Image
                      src={
                        croppedImage ||
                        URL.createObjectURL(previewImageForPost.file)
                      }
                      alt="post-image"
            ...
            <>
                        ...
                        >
                          <div className="flex flex-col gap-y-1 w-full">
                            ...
                            <p className="text-sm font-light">
                              Choosing 'Everyone' makes your posts visible to
                              all.
                            </p>
                          </div>
                        </div>
              ...
              <div className=" bg-background pt-2 p-4 mt-auto">
                <Button
                  ...
                  disabled={isPosting}
                >
                  {isPosting ? "Uploading Post..." : "Upload Post"}
                </Button>
              </div>
            </>
          )}
