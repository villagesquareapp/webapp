...
const STATS = [
  {
    icon: <HiOutlineUserGroup className="size-6 text-red-400" />,
    label: "Total Viewers",
    value: "12,500",
    change: "+18% vs last live",
  },
  {
    icon: (
      <Image
        src="/images/gifts/vs-coin.svg"
        alt="Cowries"
        className=""
        width={30}
        height={30}
    ...
    value: "CWR 467",
    change: "+22.3% vs last live",
  },
  {
    icon: <LuGift className="size-6 text-green-400" />,
    label: "Gifts Received",
    value: "1,246",
    change: "+7.2% vs last live",
  },
  {
    icon: <MdOutlinePersonAdd className="size-6 text-blue-400" />,
    label: "New Followers",
    value: "541",
    change: "+15% vs last live",
  },
];

const ENGAGEMENT = [
  {
    icon: <FaHeart className="size-4 text-pink-500" />,
    label: "Likes",
    value: "25,600",
    change: "+23.1%",
  },
  {
    icon: <FaComment className="size-4 text-[#8E8E93]" />,
    label: "Comments",
    value: "5,410",
    change: "+17.2%",
  },
  {
    icon: <FaShareAlt className="size-4 text-green-400" />,
    label: "Shares",
    value: "1,204",
    change: "+14.6%",
  },
  {
    icon: <MdOutlineAccessTime className="size-4 text-blue-400" />,
    label: "Average\nWatch Time",
    value: "42m 18s",
    change: "+13.1%",
  },
...
const EARNINGS = [
  {
    icon: <BiDiamond className="size-5 text-white/70" />,
    label: "Total Diamonds",
    value: "45,890",
    bg: "bg-white/5",
  },
  {
    icon: <LuWallet className="size-5 text-green-400" />,
    label: "Total Earnings",
    value: "$453,00",
    bg: "bg-white/5",
  },
  {
    icon: <LuGift className="size-5 text-cyan-400" />,
...
export default function LiveSummary2Page() {
  return (
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-6 py-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-[#8E8E93] mt-0.5">
              Here&apos;s how your live stream performed.
            </p>
          </div>
          <div>
            {/* View Replay */}
            <div className="flex justify-end mb-5">
              <Button
                variant="outline"
                className="rounded-full border-white/10 text-white/80 text-sm h-9 px-4 gap-2"
              >
        ...
        <div className="grid grid-cols-4 gap-3 mb-4">
          {STATS.map((stat, i) => (
            <div
              key={i}
              className="bg-[#232325] rounded-xl px-4 py-4 flex items-center gap-3"
            >
                ...
                <div key={i}>
                  <div className="flex items-center justify-between py-2.5">
                    <div className="flex items-center gap-3">
                      {item.icon}
                      <span className="text-white/80 text-sm whitespace-pre-line leading-tight">
                        {item.label}
                      </span>
                    </div>
          ...
          <div className="bg-[#232325] rounded-xl p-5 h-[330px] flex flex-col">
            ...
            <div className="flex flex-col gap-3 flex-1">
              {EARNINGS.map((item, i) => (
                <div
                  key={i}
                  className={`${item.bg} rounded-lg px-3 py-3 flex items-center gap-3`}
                >
        ...
        {/* Gifts Highlights */}
        <div className="bg-[#232325] rounded-xl p-5 mb-8">
          <h3 className="text-white font-semibold text-sm mb-4">
            Gifts Highlights
          </h3>
          <div className="flex items-center justify-around gap-6">
            {GIFTS.map((gift, i) => (
              <div key={i} className="flex flex-col items-center gap-1.5">
                ...
                <span className="text-white/80 text-[11px] text-center leading-tight">
                  {gift.name}
                </span>
        ...
        {/* Footer — only the disclaimer on the left */}
        <div className="flex items-center py-4 border-t border-white/5">
          <p className="text-[#8E8E93] text-xs">
            All stats are estimated and may be delayed.
          </p>
        </div>
      </div>

      {/* Right Sidebar: Top Gifters + Feedback */}
      <div className="w-[400px] shrink-0 border-l border-white/5 overflow-y-auto no-scrollbar p-5 flex flex-col">
        <div className="bg-[#232325] rounded-xl p-5">
          <h3 className="text-white font-semibold text-sm mb-5">Top Gifters</h3>
            ...
            {TOP_GIFTERS.map((gifter, i) => (
              <div key={i} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <CustomAvatar
                    src={
                      "https://cdn-assets.villagesquare.io/profile_pictures/default_user.png"
                    }
                    name={gifter.name}
                    className="size-10"
                  />
                  <div className="flex items-center gap-2">
                    ...
                    {gifter.rank && (
                      <span className="text-white text-[10px] bg-[#1F2733] px-2 py-0.5 rounded-full font-medium">
                        {gifter.rank}
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-1.5">
                  <Image
                    src={"/images/gifts/villagesquare_coin.svg"}
                    alt="VS_coin"
                    width={17}
                    height={17}
                  />
          ...
          <button className="w-full mt-4 text-center border border-white/10 rounded-xl py-2.5 text-[#8E8E93] text-sm hover:bg-white/5 transition-colors">
            See more...
          </button>
        </div>
